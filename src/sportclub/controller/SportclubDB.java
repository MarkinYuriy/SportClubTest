...
public class SportclubDB implements ISportclubRepository {
	...
	public TeamData addTeam(Map<String, Object> mapJ) {
		...
		try {
			...
			if (clubFinded.getId() == 0)
				return new TeamData("club with id " + clubId + " doesn't exist!");
	...
	public Club addClub(Club club) {
		try {
			...
			Query q = em.createQuery("SELECT c.name " + "FROM Club c WHERE c.name=:name");
	...
	public ProfileData removeProfiler(String id) {
		...
		if (pr != null) {
			...
			/*
			 * Set<Team> teams = pr.getTeams(); for(Team t:teams){ Team tm =
			 * em.find(Team.class, t.getId()); Set<Profiler> prs =
			 * tm.getProfiles(); for(Profiler p:prs){
			 * if(p.getCode().equals(pr.getCode())) prs.remove(p); }
			 * tm.setProfiles(prs); } pr.setTeams(null);
			 */
	...
	public Profiler getProfile(String subProfiler, String id) {
		...
		Query query = em
				.createQuery("select p from Profiler as p where p.id=:id and p.class=:type and p.deleted = false");
